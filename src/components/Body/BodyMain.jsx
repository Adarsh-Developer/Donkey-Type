...
// Paragraphs in the different situation....
...
const BodyMain = ({ isNumber, isPunctuation, isTimer, isFirstStart, setIsFirstStart }) => {
  ...
  const startTimer = useCallback(() => {
    console.log("Hello from startTimer");
    clearInterval(intervalRef.current);
    intervalRef.current = setInterval(() => {
      console.log("Interval executing...");
      setCountDown((prevCountDown) => {
        console.log("Previous Countdown:", prevCountDown);
        if (prevCountDown === 0) {
    ...
    console.log("Current timerRef:", timerRef.current);
  }, []);
  ...
  return (
    <div className="typingTest flex flex-col gap-5">
      <div className="textModesNotice w-100% flex flex-col items-center justify-center">
        <h1 className="mb-[50px] text-[25px] text-[#646669] font-Roboto cursor-default" >Press <span className="px-[5px] py-[0] dark:bg-[#646669] bg-white dark:text-[#3c393f] text-black rounded-[2px]" >ENTER</span> to start the typing everytime!</h1>
      ...
      <div className={`text-[#e2b714] font-medium ${isFirstStart ? 'block' : 'hidden'}`}>
        <h1 className="text-xl" >{countDown}</h1>
      </div>

      <div className="words-wrapper w-[80vw]">
        <div className="font-Roboto words text-[#646669] text-[21px] tracking-[3px] cursor-default select-none">{para}</div>
      </div>
